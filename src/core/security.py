"""PASETO token management and password hashing utilities."""
...
# ── Convenience wrappers using settings.SECRET_KEY ─────────────────
...
def create_paseto_token(
    *,
    ...
    """Create access token using the configured paseto_secret_key."""
...
def decode_paseto_token(token: str) -> dict[str, Any]:
    """Decode a PASETO token created with the configured paseto_secret_key.
    Returns payload with 'sub' and 'tenant_id' keys for downstream consumption.
    """
    from src.core.config import settings

    payload = decode_token(secret=settings.paseto_secret_key, token=token)
    # Normalise: expose 'tenant_id' alias for 'tid'
